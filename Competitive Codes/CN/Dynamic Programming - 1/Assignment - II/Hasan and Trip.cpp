...
int main(){
    ...
    index[1]=1;
    dp[0] = 0;
    for(int i=1;i<n;i++) dp[i] = -DBL_MAX; 
    //dp[1] = dp[0] + v[1].f - distance(v[0].x,v[0].y,v[1].x,v[1].y);
    for(int i=0;i<n;i++){
        dp[i]+=v[i].f;
        //double save = -DBL_MAX;
       for(int j=i+1;j<n;j++){
           double i1 = dp[i] - distance(v[j].x,v[j].y,v[i].x,v[i].y);
           dp[j] = max(i1,dp[j]);
       }
       // dp[i] = save;
    }    
        
    //     double i2 = dp[i-2] + v[i].f - distance(v[index[i-2]].x,v[index[i-2]].y,v[i].x,v[i].y);
    //     double i1 = dp[i-1] + v[i].f - distance(v[index[i-1]].x,v[index[i-1]].y,v[i].x,v[i].y);
    //     if(i1>=i2){
    //         dp[i] = i1;
    //         index[i] = i;
    //     }else{
