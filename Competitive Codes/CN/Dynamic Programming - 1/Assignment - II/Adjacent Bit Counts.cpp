...
int solve(int n,int k,int first){
    ...
    if(n==1){
        //if(k==1) return 0;
        if(k==0) {dp[n][k][first]=1;return 1;}
        else {dp[n][k][first]=0;return 0;}
    }if(k<0) {dp[n][k][first]=0;return 0;}
    
   
    //int ans1=0,ans2=0;
    if(first==1){
      ll a = solve(n-1,k-1,1);
      ll b = solve(n-1,k,0);
        dp[n][k][first] = (a + b)%MOD;
    }
    else{ //dp[n][k][first] = (solve(n-1,k,1) + solve(n-1,k,0))%MOD;
        ll a = solve(n-1,k,1);
        ll b = solve(n-1,k,0);
        dp[n][k][first] = (a + b)%MOD;
    }
...
int main(){
    ...
    while(t--){
        ...
        int index,n,k;
        cin>>index>>n>>k;cout<<index<<" ";
        cout<<(solve(n,k,0)+solve(n,k,1))%MOD<<"\n";
    }
    return 0;
}
