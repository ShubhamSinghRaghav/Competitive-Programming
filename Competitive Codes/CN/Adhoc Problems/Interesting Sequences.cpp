...
//     return false;
// }
int coins_needed(int a[],int n,int value,int case1,int case2){
    int up=0,dn=0;
    for(int i=0;i<n;i++){
        if(a[i]<value){
            up += value-a[i]; 
    ...
    if(up<dn) return INT_MAX;
    up = up - dn;
    int ans = up*case2 + dn*case1;
    return ans;
}
int main(){
    int n,k,l;
    ...
    sort(a,a+n);
    // int lb=0,ub=n-1,pos=0;
    // while(lb<=ub){
    //     int mid = (lb+ub)/2;
    //     bool flag = check(a,n,a[mid]);
    //     if(flag){
    //         pos = mid;
    //         ub = mid-1;
    //     }else{
   ...
   // cout<<a[pos]<<"\n";
    ...
    for(int i=a[0];i<=a[n-1];i++){
        int save = coins_needed(a,n,i,k,l);
        miner = min(miner,save);
    }
    cout<<miner;
    return 0;
}